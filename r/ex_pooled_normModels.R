...
#the model matrix
X<-model.matrix(~x + y,data = data.frame(x = runif(N,-2,2),
                                     y = runif(N,-2,2)))
y<-vector(length = N)
for(n in 1:N){
...
m_hier <- sampling(mod3, data=list(N=N, J=J, K=K, id=id, X_ij=X, y_i=y),
                   iter = 4000, chains = 4, cores = 1,
                   control = list(adapt_delta = 0.95, max_treedepth = 20))
...
mod3 <- stan_model(here::here("r/exampleStanModels/varyIntFixB_lm.stan"))
